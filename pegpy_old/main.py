...
def parse(opt, conv=None):
    # parse({inputs:hoge.txt, grammar:hoge.tpeg})
    g = load_grammar(opt)
    #g = Grammar(hoge).load('hoge.tpeg')
    parser = switch_generator(opt)(g)
    #parser = nez(g)
    #nez defined in (peg.py:10)
    #nez.setting('nez')
    #pg = pegpy.parser
    #nez(g) = pg.generate_parser(pg.generate(g, 'nez'), conv=None))
    #parser = nez(g) = generate_parser(generate(g,'nez'),conv) //setted by 'nez'
    inputs = opt['inputs']
    if len(inputs) == 0:
        try:
            while True:
