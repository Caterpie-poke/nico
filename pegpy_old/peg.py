...
def nez(p, conv = None):
    #p = Grammar(hoge).load('hoge.tpeg')
    pg.setting('nez')
    #pegpy.parser.setteing('nez')
    #nez = pegpy.parser.generate_parser(pg.generate(p, 'nez'), conv)
    return pg.generate_parser(pg.generate(p, 'nez'), conv)
    #nez(g) = generate_parser(generate(g,'nez'), None) //setted by 'nez'
