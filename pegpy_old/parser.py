...
def p_Any(px):
    if px.pos < px.length:
        px.pos += 1
        while px.pos < px.length and 128 <= px.inputs[px.pos] and px.inputs[px.pos] < 128+64:
            px.pos += 1
...
#this is parser
#p = grammar
# f is overrided with 'nez'
def generate(p, f = 'dasm'):
    #pg.generate(g,'nez')
    #p = g = Grammar(hoge).load('hoge.tpeg')
    setting(f)
    #setted 'nez'
    if not isinstance(p, ParsingExpression): # Grammar so False
        p = Ref(p.start().name, p)
    return getattr(p, f)()

#generate_parser(generate(g,'nez'),conv)(str)
def generate_parser(f, conv = None):
    def parse(s, urn = '(unknown)', pos = 0):
        ...
        if not f(px):
        #Parsing is here?
        #getter(g->p:ParsingExpression,'nez')()(px:ParserContext(inputString,(unknown),0))
        #parse(str) = generate(g,'nez')(px)
